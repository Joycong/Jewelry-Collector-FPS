...
public class ExplosionBarrel : InteractionObject // InteractionObject 상속
{
    ...
    public override void TakeDamage(int damage) // 추상 메소드 재정의
    {
        currentHP -= damage;

        if (currentHP <= 0 && isExplode == false)
        {
            StartCoroutine("ExplodeBarrel"); // 꽝!
        }
    }
    ...
    private IEnumerator ExplodeBarrel()
    {
        ...
        foreach (Collider hit in colliders)
        {
            ...
            if (player != null)
            {
                player.TakeDamage(50); // 플레이어 체력 50 감소
                continue;
            }
            ...
            if (enemy != null)
            {
                enemy.TakeDamage(300); // 적 체력 300 감소
                continue;
            }
